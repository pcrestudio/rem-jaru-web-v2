...
const Sidebar: FC<SidebarProps> = ({ user }) => {
  ...
  return (
    <aside
      className={`${minWidth} sidebar-transition bg-cerulean-900 px-8 py-4 border border-b-gray-500 h-screen border-l-0 border-r-0 border-t-gray-500 flex relative transition-all`}
    >
      <div
        ...
        {menuOptions.map(
          ({ title, Icon, role, redirectTo, isMultiple }, index) =>
            role.includes(user?.role) && (
              <ListboxItem
                key={index}
                className={`${isCollapsed ? "flex-col" : "flex-row"} text-white ${validatePathname(pathname, redirectTo) ? "bg-cerulean-950 text-white" : "data-[hover=true]:bg-transparent"} data-[hover=true]:bg-cerulean-950 data-[hover=true]:text-white`}
                startContent={<Icon />}
                href={redirectTo}
              >
