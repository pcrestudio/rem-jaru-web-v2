...
export interface FilterStatusAutocompleteProps extends ReactiveFieldProps {
  onChange: (event: any, value: string | number | object) => void;
}

const FilterStatusAutocomplete: FC<FilterStatusAutocompleteProps> = ({
  name,
  label,
  className,
  isRequired,
  ...
  const list = data?.find(
    (master) => master.slug === MasterOptionConfig.status,
  );
...
export default FilterStatusAutocomplete;
